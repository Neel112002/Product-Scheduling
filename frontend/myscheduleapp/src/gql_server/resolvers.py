...
def resolve_user_role(user: AppUser, *_):
    emp = next((e for e in user.employments if e.status == "active" and e.role), None)
    if not emp:
...
def resolve_user_company(user: AppUser, *_):
    emp = next((e for e in user.employments if e.status == "active" and e.company), None)
    return emp.company if emp else None

@user_object.field("primaryLocation")
def resolve_user_primary_location(user: AppUser, *_):
    emp = next((e for e in user.employments if e.status == "active" and e.location), None)
...
def resolve_shift_role(shift: Shift, *_):
    # Schema defines role as String on Shift
...
def resolve_my_locations(_, info):
    ...
    return (
        ...
        .filter(Employment.user_id == user.user_id, Employment.status == "active")
        .all()
    )
...
def resolve_shifts_by_location(_, info, locationId: int):
    """Returns published shifts for a location. User must be employed there."""
    user = require_user(info)
...
def resolve_team_members(_, info, locationId: int):
    ...
    try:
        return team_service.get_team_members(
            requester_id=user.user_id, location_id=locationId
        )
...
def resolve_location_roles(_, info, locationId: int):
    user = require_user(info)
    emp = Employment.query.filter_by(
        user_id=user.user_id, location_id=locationId, status="active"
    ).first()
    if not emp:
        raise GraphQLError("Forbidden")
...
def resolve_create_role(_, info, locationId: int, name: str):
    ...
    role = Role(name=name.strip(), location_id=locationId, is_system=False, created_by=user.user_id)
    db.session.add(role)
    db.session.commit()
    return role
...
def resolve_delete_role(_, info, roleId: int):
    ...
    emp = Employment.query.filter_by(
        user_id=user.user_id, location_id=role.location_id, status="active"
    ).first()
    if not emp or emp.role.name.lower() != "owner":
...
def resolve_update_user_role(_, info, empId: int, roleId: int):
    ...
    current_emp = Employment.query.filter_by(
        user_id=user.user_id, location_id=target_emp.location_id, status="active"
    ).first()
    if not current_emp:
        raise GraphQLError("Forbidden")
...
def resolve_send_onboarding_invite(_, info, email: str, locationId: int, position: Optional[str] = None):
    ...
    if not emp or emp.role.name.lower() not in ("owner", "manager"):
        raise GraphQLError("Only owner or manager can invite staff")

    existing = OnboardingInvite.query.filter_by(
        email=email, location_id=locationId, status="pending"
    ).first()
    if existing:
        raise GraphQLError("Invite already pending for this email")

    # Use OnboardingService to create invite properly
